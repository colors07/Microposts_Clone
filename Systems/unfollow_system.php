...
if ($_POST["action"] == "unfollow") {
    ...
    $statement->bindParam(":followed_user_id", $_POST["unfollowed_user_id"]);
    $statement->execute();
    $records = $statement->fetchAll();
    
    if (!empty($records)) {
        $sql = "DELETE FROM follow WHERE following_user_id = :following_user_id AND followed_user_id = :followed_user_id";
        $statement = $database->prepare($sql);
        $statement->bindParam(":following_user_id", $_SESSION["user_id"]);
        $statement->bindParam(":followed_user_id", $_POST["unfollowed_user_id"]);
        $statement->execute();
